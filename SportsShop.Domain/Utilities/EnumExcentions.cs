...
namespace SportsShop.Domain.Utilities
{
    public static class EnumExtensions
    {
        ...
        public static IEnumerable<SelectListItem> GetAllEnumSelectListItemValueInt<TBaseType>()
            where TBaseType : Enum, new()
        {
            foreach (TBaseType item in Enum.GetValues(typeof(TBaseType)))
            {
                yield return new SelectListItem()
                    { Value = (Convert.ToInt32(item)).ToString(), Text = item.ToDisplay() };
            }
        }
