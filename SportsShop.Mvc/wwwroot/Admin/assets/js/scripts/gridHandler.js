$(document).ready(() => insertData(JSON.parse($("#grid-container").attr("data"))));

const insertData = (data) => {
    let cards = "";
    data.map((item) => {
        cards += `
    <div id="Project_${item.ProjectId}" class="col-lg-2">
             
                <div class="card grid-shadow">
                    <div class="card-body p-0">                                        
                        
                        <div class="text-center project-card">
                                <a  href="/Project/ProjectContent/${item.ProjectId}">
                            <img src="/project/${item.ProjectImage}" loading="lazy" alt="${item.ProjectName}"  class="mx-auto d-block mb-3 w-100"> 
                                   </a>
                            <div class="row">
                             <div class="col-md-8">
  <a  href="/Project/ProjectContent/${item.ProjectId}">
                               <h3 class="project-title ">${item.ProjectName}</h3>
</a>
                             </div>
                             <div class="col-md-4">
                              <div class=" d-flex justify-content-end">                                        
                                <div class="dropdown d-inline-block" style="margin: 10px 0;">
                                    <a class="nav-link dropdown-toggle arrow-none" id="dLabel1" data-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
                                        <i class="fas fa-ellipsis-v font-20 text-muted"></i>
                                    </a>
                                    <div class="dropdown-menu" aria-labelledby="dLabel1">
                                  
                                        <a class="dropdown-item" href="/Project/ProjectCreateOrUpdate/${item.ProjectId}">ویرایش پروژه </a> 
                                        <a class="dropdown-item"  onclick="DeleteProject(${item.ProjectId})">حذف پروژه</a> 
                                        
                                    </div>
                                </div>
                            </div> 
                             </div>
                           
                           </div>
                         <a  href="/Project/ProjectContent/${item.ProjectId}">
                            <p class="text-muted"><span class="text-secondary font-14"><b>پیشرفت پروژه : </b></span> % ${item.Progress.toFixed(2)}
                            </p>
                          
                       
                         </a>
                        </div>                                                                      
                    </div><!--end card-body-->
                    
                </div><!--end card-->
              
            </div><!--end col-->

`;
    });

    document.getElementById("grid-container").innerHTML = cards;
};
$("#mySearchText").keyup(function () {
    let data = JSON.parse($("#grid-container").attr("data"));
    filtered = data.filter((item) => item.ProjectName.includes($(this).val()));






    insertData(filtered);
});
